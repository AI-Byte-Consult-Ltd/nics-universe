...
CORE_TRAITS = [
    ...
    "stealth", "endurance", "insight", "willpower", "fertility",
    "discipline", "instinct", "vitality", "harmony", "independence",
]
...
class Genome:
    ...
    def random_genome(min_genes: int = 6, max_genes: int = 12) -> "Genome":
        ...
        selected = random.sample(CORE_TRAITS, num)
        genes = {trait: round(random.uniform(5, 95), 1) for trait in selected}
        return Genome(genes=genes)

    def mutate(self, rate: float = 0.10, strength: float = 0.12) -> "Genome":
    ...
    def from_dict(cls, d: dict) -> "Genome":
        return cls(genes=dict(d.get("genes", {})))
